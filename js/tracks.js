...
//	bufferdrawer.appendChild(canvas);
  ...
  var startFunc = (e) => {
    e.preventDefault();
    console.log('HEYO', e)
    startPosition = e.type === 'touchstart' ? [e.layerX, e.layerY] : [e.offsetX, e.offsetY];
  }
  var moveFunc = (e) => {
    e.preventDefault();
    var newPosition = e.type === 'touchmove' ? [e.layerX, e.layerY] : [e.offsetX, e.offsetY];
    console.log(startPosition, newPosition);
    if(startPosition){
      var diff = startPosition[1] - newPosition[1];
      var rate = diff / 100;
      console.log(rate);
  ...
  platter.addEventListener('mousedown', startFunc);
  platter.addEventListener('mousemove', moveFunc);
  platter.addEventListener('mouseout', doneFunc);

  platter.addEventListener('touchstart', startFunc);
  platter.addEventListener('touchmove', moveFunc);
  platter.addEventListener('touchend', doneFunc);
